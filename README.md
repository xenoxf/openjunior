...
> **Glenker nace como un fork de OpenChamber** con una visión distinta: llevar la experiencia de los agentes de código al usuario común, no solo al dev que ya vive en terminal. Mejoras de UX, catálogo de MCPs, Skills reutilizables e integraciones con el ecosistema Google.  
...
Glenker es una interfaz rica para agentes de código AI. Review diffs, gestiona agentes, corre servidores y mantén el panorama completo mientras tu AI codea. Desde el navegador, el escritorio o tu celular.

![Glenker Chat](docs/references/chat_example.png)
...
- Notificaciones nativas para alertas de tareas mientras Glenker está oculto
...
- Conexiones deep-link para unirte a Glenker remoto desde un link
...
- Temas personalizados via JSON en `~/.config/glenker/themes/` — hot reload, sin reinicio
...
Visita **[glenker.vercel.app](https://glenker.vercel.app)** y descarga la versión para tu sistema operativo.

### CLI (Web + PWA)
_Requiere Node.js 22+_

```bash
curl -fsSL https://raw.githubusercontent.com/xenoxf/glenker/main/scripts/install.sh | bash
glenker --ui-password tu-contraseña
```
...
```bash
glenker --port 8080                    # Puerto personalizado
glenker --lan --port 3000              # Escuchar en LAN (0.0.0.0)
glenker --ui-password secret           # UI protegida con contraseña
glenker startup enable                 # Iniciar al login como servicio
glenker tunnel help                    # Comandos de túneles
glenker tunnel providers               # Proveedores disponibles
glenker tunnel start --provider cloudflare --mode quick --qr
glenker connect-url --port 3000        # Agregar servidor a Desktop
glenker connect-url --server http://host:3000 --qr
glenker logs                           # Ver logs de la instancia
OPENCODE_PORT=4096 OPENCODE_SKIP_START=true glenker
glenker stop                           # Detener servidor
glenker update                         # Actualizar a última versión
```

</details>

### Desktop (macOS + Windows)
Descarga desde [Releases](https://github.com/xenoxf/glenker/releases).
...
[Service]
Type=simple
ExecStart=glenker serve --port 3000 --host 0.0.0.0 --ui-password tu-password --foreground
...
| `GLENKER_TUNNEL_MODE` | `quick`, `managed-remote` o `managed-local` |
...
- [ ] Desacoplar Glenker de OpenCode — soporte multi-proveedor (Anthropic, OpenAI, Gemini, modelos locales)
