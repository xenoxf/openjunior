...
Rendering and hosting still happen in `glenker-website` (`apps/docs`).
...
- creates `glenker-docs-source-<sha>.tar.gz`
...
to `glenker-website` so it re-syncs and redeploys the docs. This fires on
...
- `GLENKER_WEBSITE_REPO_TOKEN` — a token with `contents: write` (classic
  `repo` scope, or fine-grained with Contents: read & write) on
  `glenker/glenker-website`. **Without it the dispatch step is skipped**
...
- `source_repo`
- `source_ref` — the ref the website checks out from `glenker` (`main` on a
  push, the tag on a release)
- `archive_name`

`glenker-website`'s `deploy-docs.yml` listens for this event
(`repository_dispatch: types: [docs_source_updated]`), checks out
`glenker` at `source_ref`, runs `docs:sync`, builds `apps/docs`, and deploys
to Cloudflare Pages. That repo needs its own secrets: `GLENKER_REPO_TOKEN`
