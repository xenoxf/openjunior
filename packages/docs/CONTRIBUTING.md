...
This package is docs content source-of-truth for Glenker.
...
- Lead with the task, not background. The first line should say what the page is
  for ("Use `glenker tunnel` to expose a running Glenker instance.").
...
- A basic page should fit in a screen or two. Long, dense reference pages (like
  ...
  this page if you run Glenker behind...").
...
```mdx
3. Run `glenker --ui-password be-creative-here`.
4. Open the printed URL (usually `http://localhost:3000`).

You should land on the Glenker session list. If you see it, the server is
running.
```
...
- Explain a term the first time it appears, in parentheses, in everyday words:
  - good: start a tunnel (a public link to your local Glenker)
...
2. Add frontmatter at top:
   ...
   ---
   title: Remote Access
   description: Access Glenker from outside your local network.
   ---
   ```
...
The docs are translated into the same languages the Glenker app ships in.
...
`glenker-website/apps/docs/astro.config.mjs` (`locales`). If a language is
...
---
title: Встановлення
description: Встановіть Glenker для десктопа, вебу або VS Code.
---
```
...
- brand and product nouns: Glenker, OpenCode, VS Code, PWA, GitHub, Discord,
  macOS, SSH
...
## Sync into glenker-website

`glenker-website` renders/deploys docs via Starlight in `apps/docs`.

After docs content updates here:

1. copy `packages/docs/content/docs/*` -> `glenker-website/apps/docs/src/content/docs/*`
   (this is recursive, so locale folders like `uk/` and `zh-cn/` carry over with
   no extra steps)
2. map `packages/docs/sidebar.config.json` into `glenker-website/apps/docs/astro.config.mjs` sidebar
