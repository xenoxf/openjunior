...
![VS Code Extension](https://github.com/btriapitsyn/openjunior/raw/HEAD/packages/vscode/extension.jpg)

**Like the extension? There's also a [desktop app and web version](https://github.com/btriapitsyn/openjunior) with even more features.**
...
Plus everything from the shared OpenJunior UI: branchable timeline, smart tool UIs, voice mode, Git workflows, and more.

## Commands

| Command | Description |
|---------|-------------|
| `OpenJunior: Focus Chat` | Focus the chat panel |
| `OpenJunior: New Session` | Start a new chat session |
| `OpenJunior: Open Sidebar` | Open the OpenJunior sidebar |
| `OpenJunior: Open Agent Manager` | Launch parallel multi-model runs |
| `OpenJunior: Open Session in Editor` | Open current or new session in an editor tab |
| `OpenJunior: Settings` | Open extension settings |
| `OpenJunior: Restart API Connection` | Restart the OpenCode API process |
| `OpenJunior: Show OpenCode Status` | Debug info for development or bug reports |

### Right-click menu

Select code in the editor, right-click, and find the **OpenJunior** submenu:
...
|---------|---------|-------------|
| `openjunior.apiUrl` | _(empty)_ | URL of an external OpenCode API server. Leave empty to auto-start a local instance. |
| `openjunior.opencodeBinary` | _(empty)_ | Absolute path to the `opencode` CLI binary. Useful when PATH lookup fails. Requires window reload to apply. |
...
- `OPENJUNIOR_VSCODE_BIN=cursor bun run vscode:dev`
- `OPENJUNIOR_VSCODE_DEV_WORKSPACE=/path/to/workspace bun run vscode:dev`
...
Install locally: `code --install-extension packages/vscode/openjunior-*.vsix`
