...
const preferredHmrUiPort = Number(process.env.GLENKER_HMR_UI_PORT || '5173');
const preferredHmrApiPort = Number(process.env.GLENKER_HMR_API_PORT || '3901');
...
function spawnProcess(command, args, options = {}) {
  ...
  return spawn(spawnCommand, spawnArgs, {
    cwd: repoRoot,
    env: { ...process.env, GLENKER_ELECTRON_DEV: '1' },
    stdio: 'inherit',
...
async function main() {
  const useBundledUi = process.env.GLENKER_ELECTRON_USE_BUNDLED_UI === '1';
  let devServer = null;
  let hmrApiPort = '';
  let hmrUiPort = '';
  ...
  if (useBundledUi) {
    ...
    devServer = spawnProcess('node', ['./scripts/dev-web-hmr.mjs'], {
      env: {
        ...process.env,
        GLENKER_ELECTRON_DEV: '1',
        GLENKER_HMR_UI_PORT: hmrUiPort,
        GLENKER_HMR_API_PORT: hmrApiPort,
  ...
  const electron = spawnProcess('npx', ['electron', './main.mjs'], {
    ...
    env: {
      ...process.env,
      GLENKER_ELECTRON_DEV: '1',
      ...(useBundledUi ? { GLENKER_ELECTRON_USE_BUNDLED_UI: '1' } : {}),
      GLENKER_HMR_UI_PORT: hmrUiPort,
      GLENKER_HMR_API_PORT: hmrApiPort,
      GLENKER_DISABLE_PWA_DEV: '1',
    },
