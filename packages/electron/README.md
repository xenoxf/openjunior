...
Electron desktop runtime for OpenJunior on macOS and Windows.

This package owns the native shell: windows, menus, deep links, native notifications, auto-updates, host switching, SSH connections, tunnel helpers, and packaged desktop builds. The web UI and OpenJunior server logic still live in `packages/web` and shared React UI lives in `packages/ui`.

## How It Runs

Desktop starts the OpenJunior web server in the same Electron main process. There is no separate sidecar subprocess for the OpenJunior server.

`main.mjs` imports `@openjunior/web/server/index.js` and calls `startWebUiServer()`. The Electron window then loads the UI from the local server in development, or from packaged `resources/web-dist` assets in packaged builds.

The preload bridge exposes desktop-only APIs to the web UI through `window.__OPENJUNIOR_DESKTOP__`. Privileged commands are checked in `main.mjs`, not only in the UI.
...
| Variable | Use |
|----------|-----|
| `OPENJUNIOR_ELECTRON_DEV=1` | Marks the runtime as desktop development mode |
| `OPENJUNIOR_ELECTRON_USE_BUNDLED_UI=1` | Uses staged web assets instead of the HMR dev server |
| `OPENJUNIOR_HMR_UI_PORT` | Preferred Vite UI port for desktop dev, default `5173` |
| `OPENJUNIOR_HMR_API_PORT` | Preferred API port for desktop dev, default `3901` |
| `OPENJUNIOR_RUNTIME=desktop` | Set by Electron before starting the web server |
| `OPENJUNIOR_DESKTOP_NOTIFY=true` | Enables desktop notification flow in the web server |
| `OPENJUNIOR_SKIP_API_COMPRESSION=true` | Defaulted by Desktop to reduce local CPU overhead |
...
2. Add the real command handling in `main.mjs` under `openjunior:invoke`.
...
Electron uses `electron-log`. In development, console logs are also visible in the terminal. In packaged apps, logs are written through the platform log path for the `OpenJunior` app name.

Development builds use a separate user data directory named `OpenJunior Dev`, so dev state does not overwrite normal packaged app state.
...
- Keep `@openjunior/web`, `bun-pty`, `node-pty`, and native modules external in `bundle-main.mjs`; bundling them can break Electron startup.
