...
export type UnifiedCatalogItemType = 'skill' | 'mcp' | 'plugin';

export type UnifiedCatalogCategory =
  | 'all'
  | 'files'
  | 'tools'
  | 'chat'
...
export interface UnifiedCatalogConfigField {
  key: string;
  label: string;
...
export interface UnifiedCatalogItem {
  ...
  installSteps?: string[];
  configFields?: UnifiedCatalogConfigField[];
}

export interface UnifiedCatalogSourceStatus {
  id: string;
  label: string;
  online: boolean;
  cached: boolean;
...
export interface UnifiedCatalogResponse {
  ...
  sourceStatus?: Record<string, { online: boolean; itemCount: number; cached: boolean }>;
  total?: number;
  error?: { kind: string; message: string };
}

export interface UnifiedCatalogSourcesResponse {
  ok: boolean;
  sources?: UnifiedCatalogSourceStatus[];
  error?: { kind: string; message: string };
}

export interface UnifiedCatalogItemResponse {
  ok: boolean;
