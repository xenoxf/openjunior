...
export const SettingsView: React.FC<SettingsViewProps> = ({ onClose, forceMobile, isWindowed, visiblePageSlugs, initialMobileStage = 'nav' }) => {
  ...
  const basicPages = React.useMemo(() => {
    return sortedFilteredPages.filter((page) => BASIC_PAGES.includes(page.slug));
  }, [sortedFilteredPages]);

  const advancedPages = React.useMemo(() => {
    return sortedFilteredPages.filter((page) => ADVANCED_PAGES.includes(page.slug));
  }, [sortedFilteredPages]);
                ...
                {advancedPages.length > 0 && (
                  <>
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 w-full px-2 py-1.5 text-muted-foreground/70 hover:text-foreground hover:bg-interactive-hover rounded-md transition-colors"
                    >
                      <Icon name={advancedExpanded ? ('chevron-down' as IconName) : ('chevron-right' as IconName)} className="h-4 w-4 shrink-0 text-muted-foreground/70 transition-transform duration-150" />
                      <span className="typography-ui-label font-medium text-foreground">{t('settings.view.advanced.title')}</span>
                    </button>
                    {advancedExpanded && (
