export const createTunnelRoutesRuntime = (dependencies) => {
  ...
  const registerRoutes = (app) => {
    app.get('/api/glenker/tunnel/check', async (req, res) => {
      try {
    ...
    app.post('/api/glenker/tunnel/doctor', handleTunnelDoctor);
    app.get('/api/glenker/tunnel/doctor', handleTunnelDoctor);

    app.get('/api/glenker/tunnel/providers', (_req, res) => {
      const providers = tunnelProviderRegistry.listCapabilities();
      return res.json({ providers });
    });

    app.get('/api/glenker/tunnel/status', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.put('/api/glenker/tunnel/managed-remote-token', async (req, res) => {
      try {
    ...
    app.post('/api/glenker/tunnel/start', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.post('/api/glenker/tunnel/stop', (_req, res) => {
      let revokedBootstrapCount = 0;
      let invalidatedSessionCount = 0;
      const activeTunnelId = tunnelAuthController.getActiveTunnelId();
