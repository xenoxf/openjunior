...
const originalGlenkerWslBinary = process.env.GLENKER_WSL_BINARY;
...
afterEach(() => {
  ...
  if (typeof originalGlenkerWslBinary === 'string') {
    process.env.GLENKER_WSL_BINARY = originalGlenkerWslBinary;
  } else {
    delete process.env.GLENKER_WSL_BINARY;
  }
});
...
describe('OpenCode env runtime', () => {
  ...
  it('throws a specific error for a configured directory without an executable CLI in strict mode', async () => {
    const dir = createTempDir('glenker-opencode-dir-');
  ...
  it('applies a valid configured executable OpenCode binary', async () => {
    const dir = createTempDir('glenker-opencode-bin-');
  ...
  it('rejects WSL settings in strict mode', async () => {
    setPlatform('win32');
    const dir = createTempDir('glenker-no-wsl-');
    process.env.PATH = dir;
    process.env.SystemRoot = dir;
    process.env.WSL_BINARY = path.join(dir, 'missing-wsl.exe');
    process.env.GLENKER_WSL_BINARY = path.join(dir, 'missing-glenker-wsl.exe');
  ...
  it('does not auto-detect OpenCode from WSL fallback paths', () => {
    setPlatform('win32');
    const dir = createTempDir('glenker-wsl-opencode-');
  ...
  it('launches Windows cmd shims through cmd call without embedded quotes', () => {
    ...
    const dir = createTempDir('glenker-opencode-cmd-');
  ...
  it('resolves npm OpenCode cmd shims to the packaged Windows executable', () => {
    setPlatform('win32');
    const npmDir = createTempDir('glenker-opencode-npm-');
