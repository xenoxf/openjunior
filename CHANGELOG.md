...
- CLI: stale server PID files are checked more carefully so unrelated processes are not mistaken for an Glenker server.
...
- Sessions: new draft sessions now start from the default model and agent instead of inheriting the previous session's selection, and fall back to OpenCode's own `default_agent` (and its model) when no Glenker default is set.
...
- Web/Remote Instances: added a headless web app mode, and remote instance switching now changes the Glenker API endpoint without loading the full remote UI.
...
- Desktop: added optional password setting in Glenker sessions settings for the local Desktop server.
...
- CLI: fixed a startup regression in global npm/bun installs where wrapper or symlinked `glenker` entrypoints could exit without output on commands like `--version` or `status`.
...
- Desktop: switch between local and remote Glenker instances, plus a thinner runtime.
...
- UI: introduced a token-based theming system, 18 themes with light/dark variants, and custom user themes from `~/.config/glenker/themes`.
...
- Added queue mode toggle to Glenker settings (chat section) with persistence across runtimes.
...
- Added Glenker VS Code extension with editor integration: file picker, click-to-open in tool parts.
...
- Initial public release of Glenker web and desktop packages in a unified monorepo.
