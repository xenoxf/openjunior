...
Use this guide when running Glenker behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm Glenker works directly first.
...
- Disable gzip on the proxy if Glenker is already compressing responses.
...
- Glenker reachable directly on LAN
...
location ~ ^/api/(event|global/event|notifications/stream|glenker/events)$ {
    proxy_pass http://127.0.0.1:3000;
...
- Glenker compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by Glenker, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
