...
Use this guide when running OpenJunior behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm OpenJunior works directly first.
...
- Disable gzip on the proxy if OpenJunior is already compressing responses.
...
- OpenJunior reachable directly on LAN
...
location ~ ^/api/(event|global/event|notifications/stream|openjunior/events)$ {
    proxy_pass http://127.0.0.1:3000;
...
location = /api/openjunior/events {
    proxy_pass http://127.0.0.1:3000;
    proxy_set_header Accept "text/event-stream";
    proxy_set_header Cache-Control "no-cache";
    proxy_buffering off;
...
- OpenJunior compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by OpenJunior, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
